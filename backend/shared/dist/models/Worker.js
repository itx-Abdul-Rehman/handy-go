...
import { SERVICE_CATEGORIES, WORKER_VERIFICATION_STATUS, DAYS_OF_WEEK, DEFAULTS, } from '../constants/index.js';
/**
 * Skill Sub-Schema
 */
 ...
 */
workerSchema.statics.findByUserId = function (userId) {
    return this.findOne({ user: userId }).populate('user', '-password');
};
/**
 * Static method to find worker by CNIC
 */
workerSchema.statics.findByCNIC = function (cnic) {
    return this.findOne({ cnic });
};
/**
 * Static method to find nearby workers
 */
workerSchema.statics.findNearby = function (lat, lng, maxDistance, category) {
    const query = {
        currentLocation: {
            $nearSphere: {
                $geometry: {
