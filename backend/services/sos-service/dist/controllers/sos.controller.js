...
import { successResponse, createdResponse, errorResponse, paginatedResponse, notFoundResponse, forbiddenResponse, asyncHandler, logger, SOS, Booking, User, USER_ROLES_OBJ, SOS_PRIORITY_OBJ, } from '@handy-go/shared';
...
export const triggerSOS = asyncHandler(async (req, res) => {
    ...
    await notificationService.sendSOSConfirmation(userId, sos._id.toString(), assessment.priority);
 ...
 * GET /api/sos/:sosId
 */
export const getSOSDetails = asyncHandler(async (req, res) => {
 ...
 */
export const getActiveSOSList = asyncHandler(async (req, res) => {
    ...
    const sortedList = sosList.sort((a, b) => {
        const priorityDiff = (priorityOrder[a.priority] ?? 3) -
 ...
 */
export const assignSOS = asyncHandler(async (req, res) => {
    const { sosId } = req.params;
    const adminId = req.user.id;
    const sos = await SOS.findOneAndUpdate({ _id: sosId, status: 'ACTIVE', assignedAdmin: { $exists: false } }, {
...
export const resolveSOS = asyncHandler(async (req, res) => {
    ...
    await notificationService.sendSOSResolved(sos.initiatedBy.userId.toString(), sosId.toString(), action);
 ...
 */
export const escalateSOS = asyncHandler(async (req, res) => {
    const sosId = req.params.sosId;
    const adminId = req.user.id;
    ...
    const newPriority = (currentIndex < 3 && currentIndex >= 0 ? priorityLevels[currentIndex + 1] : undefined) ?? SOS_PRIORITY_OBJ.CRITICAL;
    ...
    await notificationService.sendEscalationNotification(seniorAdmins.map(a => a._id.toString()), {
 ...
 */
export const markFalseAlarm = asyncHandler(async (req, res) => {
    const { sosId } = req.params;
    const adminId = req.user.id;
 ...
 */
export const getSOSStats = asyncHandler(async (req, res) => {
