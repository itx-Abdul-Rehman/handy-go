 ...
 */
export const assessPriority = (
  description: string,
  context?: {
    timeOfDay?: Date;
    hasActiveBooking?: boolean;
    userHistory?: { previousSOS: number; validSOS: number };
  }
): {
  priority: string;
  confidence: number;
  ...
  const criticalMatches = tokens.filter(token =>
    CRITICAL_KEYWORDS.some(keyword => token.includes(keyword) || keyword.includes(token))
  );
  ...
  const highMatches = tokens.filter(token =>
    HIGH_KEYWORDS.some(keyword => token.includes(keyword) || keyword.includes(token))
  );
  ...
  const mediumMatches = tokens.filter(token =>
    MEDIUM_KEYWORDS.some(keyword => token.includes(keyword) || keyword.includes(token))
  );
  ...
  const isolationMatches = tokens.filter(token =>
    AGGRAVATING_FACTORS.location.isolatedKeywords.includes(token)
  );
 ...
 */
export const detectPotentialFalseAlarm = (
  description: string,
  context?: {
    bookingStatus?: string;
    timeSinceBookingStart?: number; // in minutes
  }
