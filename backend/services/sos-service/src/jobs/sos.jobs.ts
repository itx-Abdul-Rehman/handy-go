  ...
  cron.schedule('*/2 * * * *', async () => {
    try {
      ...
      for (const sos of unassignedSOS) {
        ...
        const threshold = escalationThresholds[sos.priority as string] ?? 15 * 60 * 1000;
        ...
        if (timeSinceCreation > threshold) {
          ...
          if (currentIndex < 3 && currentIndex >= 0) {
            const newPriority: string = priorityLevels[currentIndex + 1] ?? SOS_PRIORITY_OBJ.CRITICAL;
      ...
      for (const sos of activeHighPrioritySOS) {
        if (sos.assignedAdmin) {
          await notificationService.sendNotification(
            (sos.assignedAdmin as any)._id.toString(),
            {
              type: 'SOS',
