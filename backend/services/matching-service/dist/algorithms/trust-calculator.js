import { Worker, Booking, Review } from '@handy-go/shared';
import { config } from '../config/index.js';
/**
 * Calculate trust score for a worker
 */
export const calculateTrustScore = async (workerId) => {
    ...
    if (worker.cnicVerified)
        verificationBonus += 5;
    if (worker.skills.every(s => s.isVerified))
        verificationBonus += 5;
    // Calculate weighted total
    breakdown.totalScore = Math.round(breakdown.rating * weights.rating +
 ...
 */
export const batchUpdateTrustScores = async () => {
 ...
 */
const getBookingStats = async (workerId) => {
