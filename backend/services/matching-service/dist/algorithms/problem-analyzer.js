...
const typedServiceKeywords = serviceKeywords;
/**
 * Analyze problem description to detect services and urgency
 */
export const analyzeProblem = (problemDescription, providedCategory) => {
    ...
    if (providedCategory && SERVICE_CATEGORIES.includes(providedCategory)) {
 ...
 */
const determineUrgency = (text, category) => {
 ...
 */
const generateSuggestedQuestions = (category, text) => {
    const questions = [];
    switch (category) {
        case 'PLUMBING':
